...
const User: React.FC<UserProps> = ({ user, slug }) => {
    ...
    const guest = authUser?.slug !== user.slug
    ...
    const closeModal = () => {
        ...
        setNewSlug(authUser?.slug!)
        setNewDesc(authUser?.description!)
    }
    ...
    // const { data: image } = useSWR(`https://frontend-test-api.yoldi.agency/api/image`, fetcher)
    ...
    useEffect(() => {
        setWTF((visible && window.innerWidth <= 600) || (visible && window.innerHeight <= 580))
    }, [visible])
    ...
    return (
        <>
            ...
            {!wtf && (
                <div>
                    <div className={styles.user_page} style={{ overflow: 'hidden' }}>
                        <div
